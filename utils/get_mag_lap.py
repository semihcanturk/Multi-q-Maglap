...
from torch_geometric.utils import add_self_loops, remove_self_loops, scatter, to_dense_adj
...
def get_mag_laplacian(
    ...
    if dynamic_q: # normalize q using longest path distance
        #edge_list = [(e[0].item(), e[1].item()) for e in edge_index.T]
        #G = nx.from_edgelist(edge_list, create_using=nx.DiGraph)
        #l = len(nx.dag_longest_path(G)) - 1
        adj_matrix = to_dense_adj(edge_index)
        l = (adj_matrix != adj_matrix.transpose(1, 2)).sum() / 2
        q /= max(min(l, num_nodes), 1)
...
class AddSingularValuePE(BaseTransform):
    ...
    def forward(self, data: Data) -> Data:
        ...
        # construct adjacency matrix instead of Laplacian
        L = to_scipy_sparse_matrix(edge_index, edge_weight, num_nodes)
        #try:
        #    U, eig_vals, V = eig_fn(
        #        L,
        ...
        U, eig_vals, Vt = svd(L.toarray())
        V = Vt.T
        # for SVD, we use the largest singular values
        #sort = eig_vals.argsort()[:self.k]
        eig_vals = eig_vals[0:self.k]
        U = U[:, 0:self.k] * np.sqrt(eig_vals)
        V = V[:, 0:self.k] * np.sqrt(eig_vals)
        ...
        pe = torch.cat([U[:, None, :], V[:, None, :]], dim=1)
        sign = -1 + 2 * torch.randint(0, 2, (self.k, ))
        pe *= sign
